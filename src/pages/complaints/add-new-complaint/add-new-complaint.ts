...
image:any='';
  ...
  fileChange(img)
 {
  ...
  genrateCompliant(){
    ...
    console.log("complaint data send",this.complaint_data);
  ...
  submit()
  {
    ...
    uploadVideo()
    {
      ...
      fileTransfer.upload(this.videoId,"http://apps.abacusdesk.com/Bathcare/dd_api/app/uploadVideos.php", options1)
      .then((data :any) => {

        var d = JSON.parse(data.response)
        console.log("video data response",data.response);
        console.log("status console=====>",data.response.status);
